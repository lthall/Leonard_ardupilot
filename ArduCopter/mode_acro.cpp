    ...
    if (g.acro_trainer != (uint8_t)Trainer::OFF) {
        ...
        if (g.acro_trainer == (uint8_t)Trainer::LIMITED) {
            ...
            if (roll_angle_rad > angle_max_rad){
                rate_ef_level_rads.x += sqrt_controller(angle_max_rad - roll_angle_rad, radians(g2.command_model_acro_rp.get_rate()) / cd_to_rad(ACRO_LEVEL_MAX_OVERSHOOT), attitude_control->get_accel_pitch_max_radss(), G_Dt);
