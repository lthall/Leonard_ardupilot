...
void ModeAcro::run()
{
    ...
    get_pilot_desired_angle_rates_rads(channel_roll->norm_input_dz(), channel_pitch->norm_input_dz(), channel_yaw->norm_input_dz(), target_roll_rads, target_pitch_rads, target_yaw_rads);
...
// get_pilot_desired_angle_rates_rads - transform pilot's normalised roll pitch and yaw input into a desired lean angle rates
// inputs are -1 to 1 and the function returns desired angle rates in centi-degrees-per-second
void ModeAcro::get_pilot_desired_angle_rates_rads(float roll_in, float pitch_in, float yaw_in, float &roll_out_rads, float &pitch_out_rads, float &yaw_out_rads)
{
    ...
    // roll expo
    rate_bf_request_rads.x = radians(g2.command_model_acro_rp.get_rate()) * input_expo(roll_in, g2.command_model_acro_rp.get_expo());

    // pitch expo
    rate_bf_request_rads.y = radians(g2.command_model_acro_rp.get_rate()) * input_expo(pitch_in, g2.command_model_acro_rp.get_expo());

    // yaw expo
    rate_bf_request_rads.z = radians(g2.command_model_acro_y.get_rate()) * input_expo(yaw_in, g2.command_model_acro_y.get_expo());
    ...
    if (g.acro_trainer != (uint8_t)Trainer::OFF) {
        ...
        // Calculate trainer mode earth frame rate command for roll
        float roll_angle_rad = wrap_PI(att_target_rad.x);
        rate_ef_level_rads.x = -constrain_float(roll_angle_rad, -ACRO_LEVEL_MAX_ANGLE * radians(0.01f), ACRO_LEVEL_MAX_ANGLE * radians(0.01f)) * g.acro_balance_roll;

        // Calculate trainer mode earth frame rate command for pitch
        float pitch_angle_rad = wrap_PI(att_target_rad.y);
        rate_ef_level_rads.y = -constrain_float(pitch_angle_rad, -ACRO_LEVEL_MAX_ANGLE * radians(0.01f), ACRO_LEVEL_MAX_ANGLE * radians(0.01f)) * g.acro_balance_pitch;
        ...
        // Calculate angle limiting earth frame rate commands
        if (g.acro_trainer == (uint8_t)Trainer::LIMITED) {
            const float angle_max_rad = copter.aparm.angle_max * radians(0.01f);
            if (roll_angle_rad > angle_max_rad){
                rate_ef_level_rads.x += sqrt_controller(angle_max_rad - roll_angle_rad, radians(g2.command_model_acro_rp.get_rate()) / ACRO_LEVEL_MAX_OVERSHOOT * radians(0.01f), attitude_control->get_accel_pitch_max_radss(), G_Dt);
            }else if (roll_angle_rad < -angle_max_rad) {
                rate_ef_level_rads.x += sqrt_controller(-angle_max_rad - roll_angle_rad, radians(g2.command_model_acro_rp.get_rate()) / ACRO_LEVEL_MAX_OVERSHOOT * radians(0.01f), attitude_control->get_accel_pitch_max_radss(), G_Dt);
            }

            if (pitch_angle_rad > angle_max_rad){
                rate_ef_level_rads.y += sqrt_controller(angle_max_rad - pitch_angle_rad, radians(g2.command_model_acro_rp.get_rate()) / ACRO_LEVEL_MAX_OVERSHOOT * radians(0.01f), attitude_control->get_accel_pitch_max_radss(), G_Dt);
        ...
        if (g.acro_trainer == (uint8_t)Trainer::LIMITED) {
            ...
            float acro_level_mix = constrain_float(1.0 - MAX(MAX(fabsf(roll_in), fabsf(pitch_in)), abs(yaw_in)), 0.0, 1.0) * ahrs.cos_pitch();
