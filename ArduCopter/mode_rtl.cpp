...
void ModeRTL::run(bool disarm_on_land)
{
    ...
    case SubMode::STARTING:
        // only reached if a restart bounced us here mid-loop; hold on the current
        // target and let advance_state() rebuild the path next loop
        climb_return_run();
        _state_complete = true;
        break;
...
bool ModeRTL::return_start()
{
    if (!wp_nav->set_wp_destination_loc(rtl_path.return_target)) {
        // failure must be caused by missing terrain data
        return false;
    }
