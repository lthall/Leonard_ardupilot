...
void ModeShipOperation::run()
{
    ...
    bool deck_radius_valid = is_positive(deck_radius);
    if (is_positive(keep_out_radius)) {
        bool approach_arc_valid = fabs(wrap_PI(radians(hotel_angle) - keep_out_center_rad)) >=  keep_out_angle_rad / 2.0;
...
// include vertical offset
        ...
        bool pos_check;
        // altitude is less than 5% of the Hotel height
        bool alt_check = fabs(-(ship.pos_ned.z + offset.z) - pos_control->get_pos_target_cm().z) < hotel_height * 0.05f;
        switch (_state) {
        case SubMode::CLIMB_TO_RTL:
