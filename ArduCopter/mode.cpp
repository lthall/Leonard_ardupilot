...
void Mode::precland_retry_position(const Vector3f &retry_pos)
{
    if (!copter.failsafe.radio) {
        ...
        if (g.land_repositioning) {
            ...
            get_pilot_desired_lean_angles_rad(target_roll_rad, target_pitch_rad, loiter_nav->get_angle_max_cd() * radians(0.01f), attitude_control->get_althold_lean_angle_max_cd() * radians(0.01f));
