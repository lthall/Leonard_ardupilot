...
void ModeDrift::run()
{
    ...
    get_pilot_desired_lean_angles_rad(target_roll_rad, target_pitch_rad, copter.aparm.angle_max * radians(0.01f), copter.aparm.angle_max * radians(0.01f));
    ...
    target_roll_rad = roll_vel_error * -DRIFT_SPEEDGAIN * radians(0.01f);
    target_roll_rad = constrain_float(target_roll_rad, -copter.aparm.angle_max * radians(0.01f), copter.aparm.angle_max * radians(0.01f));
    ...
    if (is_zero(target_pitch_rad)) {
        ...
        target_pitch_rad = pitch_vel * braker * radians(0.01f);
    } else {
        braker = 0.0f;
    }
