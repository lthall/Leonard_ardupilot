...
function move_vehicle()
    ...
    local pos_offset_NED, _, _ = vehicle:get_posvelaccel_offset()
    ...
    if vehicle:set_velaccel_offset(vel_offset_NED, Vector3f()) then
...
function update()
    ...
    if (not send_velocity_offsets and send_velocity_offsets_prev) then
        ...
        vehicle:set_posvelaccel_offset(Vector3f(), Vector3f(), Vector3f())
    end
