...
void AC_AutoTune::run()
{
    ...
    if (zero_rp_input) {
        ...
        get_poshold_attitude(desired_roll_rad, desired_pitch_rad, desired_yaw_rad);
    }
    ...
    case TuneMode::TUNING:
        ...
        if (!zero_rp_input || !is_zero(desired_yaw_rate_rads) || !is_zero(target_climb_rate_cms)) {
            ...
            if (now_ms - override_time > AUTOTUNE_PILOT_OVERRIDE_TIMEOUT_MS) {
                ...
                desired_yaw_rad = ahrs_view->yaw_sensor; // Reset yaw reference
            }
        }
...
bool AC_AutoTune::currently_level()
{
    ...
    if (fabsf(ahrs_view->roll - desired_roll_rad) > threshold_mul * cd_to_rad(AUTOTUNE_LEVEL_ANGLE_CD)) {
        return false;
    }
    if (fabsf(ahrs_view->pitch - desired_pitch_rad) > threshold_mul * cd_to_rad(AUTOTUNE_LEVEL_ANGLE_CD)) {
        return false;
    }
    if (fabsf(wrap_PI(ahrs_view->yaw - desired_yaw_rad)) > threshold_mul * cd_to_rad(AUTOTUNE_LEVEL_ANGLE_CD)) {
        return false;
    }
...
void AC_AutoTune::control_attitude()
{
    ...
    case Step::EXECUTING_TEST: {
        ...
        if (axis == AxisType::YAW || axis == AxisType::YAW_D) {
            desired_yaw_rad = ahrs_view->yaw_sensor;
        }
        break;
    }
...
void AC_AutoTune::get_poshold_attitude(float &roll_out_rad, float &pitch_out_rad, float &yaw_out_rad)
{
    ...
    Vector3f pos_error = pos_control->get_pos_estimate_NEU_cm().tofloat() - start_position;
    pos_error.z = 0;
    float dist_cm = pos_error.length();
    if (dist_cm < 10) {
    ...
    Vector2f angle_ne(pos_error.x, pos_error.y);
     ...
     */
    float target_yaw_rad = atan2f(pos_error.y, pos_error.x);
