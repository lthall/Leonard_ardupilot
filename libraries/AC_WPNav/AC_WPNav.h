...
public:
    ...
    void set_rangefinder_terrain_offset_m(bool use, bool healthy, float terrain_offset_m);
    ...
    void get_wp_stopping_point_NEU_m(Vector3f& stopping_point_m) const;
...
protected:
    ...
    float       _wp_desired_speed_ne_ms;    // desired wp speed in cm/sec
    Vector3f    _origin_neu_m;              // starting point of trip to next waypoint in cm from ekf origin
    Vector3f    _destination_neu_m;         // target destination in cm from ekf origin
