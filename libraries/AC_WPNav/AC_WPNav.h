...
public:
    ...
    // The desired state is assumed to be at rest (zero velocity and acceleration);
    // callers transitioning while moving must first brake to a stop (see
    // AC_PosControl::find_stopping_point_NED()) so that the current desired position is
    // the stopping point.
