...
bool AC_WPNav_OA::set_wp_destination_NEU_cm(const Vector3f& destination_ned_cm, bool is_terrain_alt)
{
    // Convert input from centimeters to meters and delegate to meter version
    return set_wp_destination_NED_m(destination_ned_cm.topostype() * 0.01, is_terrain_alt);
}
...
bool AC_WPNav_OA::set_wp_destination_NED_m(const Vector3p& destination_ned_m, bool is_terrain_alt)
{
    // Call base implementation to set destination and terrain-altitude flag
    const bool ret = AC_WPNav::set_wp_destination_NED_m(destination_ned_m, is_terrain_alt);
