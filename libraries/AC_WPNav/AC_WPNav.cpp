...
const AP_Param::GroupInfo AC_WPNav::var_info[] = {
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("ACCEL",       5, AC_WPNav, _wp_accel_cmss, WPNAV_ACCELERATION_MS),
...
{    
    ...
    _wp_desired_speed_ne_ms = is_positive(speed_ms) ? speed_ms : _wp_speed_cms * 0.01;
    ...
    _pos_control.set_max_speed_accel_U_m(-get_default_speed_down_ms(), _wp_speed_up_cms * 0.01, _wp_accel_z_cmss * 0.01);
    _pos_control.set_correction_speed_accel_U_mss(-get_default_speed_down_ms(), _wp_speed_up_cms * 0.01, _wp_accel_z_cmss * 0.01);

    // calculate scurve jerk and jerk time
    if (!is_positive(_wp_jerk_msss)) {
        _wp_jerk_msss.set(get_wp_acceleration_mss() * 10.0);
    }
...
{
    return set_wp_destination_NEU_cm(destination_neu_cm * 0.01, is_terrain_alt);
}
...
bool AC_WPNav::set_wp_destination_NEU_m(const Vector3f& destination_neu_m, bool is_terrain_alt)
{
    ...
    if (is_terrain_alt == _is_terrain_alt) {
        ...
        if (is_terrain_alt) {
            ...
            // new destination_neu_m is alt-above-ekf-origin, previous destination_neu_m was alt-above-terrain
            _origin_neu_m.z += origin_terr_offset_u_m;
            _pos_control.init_pos_terrain_U_m(0.0);
        }
    }

    // update destination_neu_m
    ...
    if (_flags.fast_waypoint && !_this_leg_is_spline && !_next_leg_is_spline && !_scurve_next_leg.finished()) {
        ...
        _scurve_this_leg.calculate_track(_origin_neu_m, _destination_neu_m,
                                         ...
                                         get_wp_acceleration_mss(), _wp_accel_z_cmss * 0.01,
...
bool AC_WPNav::update_wpnav()
{
    ...
    if (_check_wp_speed_change) {
        if (!is_equal(_wp_speed_cms.get(), _last_wp_speed_cms)) {
            set_speed_NE_ms(_wp_speed_cms * 0.01);
            _last_wp_speed_cms = _wp_speed_cms;
        }
    }
    if (!is_equal(_wp_speed_up_cms.get(), _last_wp_speed_up_cms)) {
        set_speed_up_ms(_wp_speed_up_cms * 0.01);
        _last_wp_speed_up_cms = _wp_speed_up_cms;
    }
    if (!is_equal(_wp_speed_down_cms.get(), _last_wp_speed_down_cms)) {
        set_speed_down_ms(_wp_speed_down_cms * 0.01);
...
bool AC_WPNav::get_terrain_offset_m(float& offset_m)
{
    ...
    case AC_WPNav::TerrainSource::TERRAIN_FROM_TERRAINDATABASE:
        ...
        if (terrain != nullptr &&
            terrain->height_above_terrain(terr_alt_m, true)) {
...
///     returns false if conversion from location to vector from ekf origin cannot be calculated
///     next_next_destination should be the next segment's destination_neu_cm
...
///     next_destination_neu_m should be set to the next segment's destination_neu_m
...
///     next_destination_neu_m.z  must be in the same "frame" as destination_neu_m.z (i.e. if destination_neu_m is a alt-above-terrain, next_destination_neu_m should be too)
bool AC_WPNav::set_spline_destination_NEU_m(const Vector3f& destination_neu_m, bool is_terrain_alt, const Vector3f& next_destination_neu_m, bool next_is_terrain_alt, bool next_is_spline)
{
    ...
    if (is_terrain_alt == _is_terrain_alt) {
        if (_flags.fast_waypoint) {
            ...
            if (_this_leg_is_spline) {
                // if previous leg was a spline we can use destination_neu_m velocity vector for origin velocity vector
        ...
        // use previous destination_neu_m as origin
        _origin_neu_m = _destination_neu_m;
    } else {
        ...
        // convert origin to alt-above-terrain if necessary
        if (is_terrain_alt) {
            // new destination_neu_m is alt-above-terrain, previous destination_neu_m was alt-above-ekf-origin
    ...
    // store destination_neu_m location
    _destination_neu_m = destination_neu_m;
    _is_terrain_alt = is_terrain_alt;

    // calculate destination_neu_m velocity vector
    Vector3f destination_vector_neu_m;
    if (is_terrain_alt == next_is_terrain_alt) {
        if (next_is_spline) {
            // leave this segment moving parallel to vector from origin to next destination_neu_m
...
/// set next destination_neu_m (e.g. the one after the current destination_neu_m) as an offset (in m, NEU frame) from the EKF origin
///     next_is_terrain_alt should be true if next_destination_neu_m.z is a desired altitude above terrain (false if its desired altitudes above ekf origin)
...
///     next_next_destination_neu_m.z  must be in the same "frame" as destination_neu_m.z (i.e. if next_destination_neu_m is a alt-above-terrain, next_next_destination_neu_m should be too)
bool AC_WPNav::set_spline_destination_next_NEU_m(const Vector3f& next_destination_neu_m, bool next_is_terrain_alt, const Vector3f& next_next_destination_neu_m, bool next_next_is_terrain_alt, bool next_next_is_spline)
{
    ...
    if (next_is_terrain_alt == next_next_is_terrain_alt) {
        if (next_next_is_spline) {
            // leave this segment moving parallel to vector from this leg's origin (i.e. prev leg's destination_neu_m) to next next destination_neu_m
...
bool AC_WPNav::get_vector_NEU_cm(const Location &loc, Vector3f &pos_from_origin_neu_cm, bool &is_terrain_alt)
{
    ...
    if (get_vector_NEU_m(loc, pos_from_origin_neu_m, is_terrain_alt)) {
        pos_from_origin_neu_cm = pos_from_origin_neu_m * 0.01;
        return true;
    }
    return false;
}
...
bool AC_WPNav::get_vector_NEU_m(const Location &loc, Vector3f &pos_from_origin_neu_m, bool &is_terrain_alt)
{
    ...
    if (loc.get_alt_frame() == Location::AltFrame::ABOVE_TERRAIN) {
        float is_terr_alt_m;
        if (!loc.get_alt_m(Location::AltFrame::ABOVE_TERRAIN, is_terr_alt_m)) {
            return false;
        }
        pos_from_origin_neu_m.z = is_terr_alt_m;
        is_terrain_alt = true;
    } else {
        is_terrain_alt = false;
        float is_terr_alt_m;
        if (!loc.get_alt_m(Location::AltFrame::ABOVE_ORIGIN, is_terr_alt_m)) {
            return false;
        }
        pos_from_origin_neu_m.z = is_terr_alt_m;
        is_terrain_alt = false;
