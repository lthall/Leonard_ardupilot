...
void AC_PosControl::set_max_speed_accel_U_cm(float vel_max_down_ms, float vel_max_up_cms, float accel_max_d_cmss)
{
    set_max_speed_accel_D_m(vel_max_down_ms * 0.01, vel_max_up_cms * 0.01, accel_max_d_cmss * 0.01);
}
...
void AC_PosControl::set_correction_speed_accel_U_cm(float vel_max_down_cms, float vel_max_up_cms, float accel_max_d_cmss)
{
    set_correction_speed_accel_D_m(vel_max_down_cms * 0.01, vel_max_up_cms * 0.01, accel_max_d_cmss * 0.01);
}
...
void AC_PosControl::set_alt_target_with_slew_cm(float alt_origin_cm)
{
    set_alt_target_with_slew_m(alt_origin_cm * 0.01);
}

// Sets target altitude in meters using jerk-limited shaping.
void AC_PosControl::set_alt_target_with_slew_m(float alt_origin_m)
{
    float pos_d_m = -alt_origin_m;
    float zero = 0;
...
void AC_PosControl::init_pos_terrain_U_cm(float terrain_alt_origin_cm)
{
    init_pos_terrain_D_m(-terrain_alt_origin_cm * 0.01);
}
...
void AC_PosControl::get_stopping_point_NE_m(Vector2p &stopping_point_ned_m) const
{
    ...
    Vector2f curr_vel = _vel_estimate_ned_ms.xy();
    curr_vel -= _vel_offset_ned_ms.xy();
    ...
    stopping_point_ned_m += (curr_vel * t).topostype();
}
