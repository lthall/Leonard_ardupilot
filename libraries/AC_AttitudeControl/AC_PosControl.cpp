...
float AC_PosControl::pos_terrain_U_scaler(float pos_terrain_u_cm, float pos_terrain_u_buffer_cm) const
{
    ...
    float pos_offset_error_u_cm = -_pos_estimate_ned_m.z * 100.0 - (_pos_target_neu_cm.z + (pos_terrain_u_cm - _pos_terrain_u_cm));
...
void AC_PosControl::soften_for_landing_NE()
{
    ...
    if (is_positive(_dt)) {
        _pos_target_neu_cm.xy() += (_pos_estimate_ned_m.xy() * 100.0 - _pos_target_neu_cm.xy()) * (_dt / (_dt + POSCONTROL_RELAX_TC));
...
void AC_PosControl::init_NE_controller()
{
    ...
    _pos_target_neu_cm.xy() = _pos_estimate_ned_m.xy() * 100.0;
    _pos_desired_neu_cm.xy() = _pos_target_neu_cm.xy() - _pos_offset_neu_cm.xy();

    _vel_target_neu_cms.xy() = _vel_estimate_ned_ms.xy() * 100.0;
...
void AC_PosControl::update_NE_controller()
{
    ...
    Vector2p comb_pos = _pos_estimate_ned_m.xy() * 100.0;
    comb_pos += _disturb_pos_ne_cm.todouble();
    ...
    // Velocity Controller
    Vector2f comb_vel = _vel_estimate_ned_ms.xy() * 100.0;
...
void AC_PosControl::init_U_controller()
{
    ...
    _pos_target_neu_cm.z = -_pos_estimate_ned_m.z * 100.0;
    _pos_desired_neu_cm.z = _pos_target_neu_cm.z - _pos_offset_neu_cm.z;

    _vel_target_neu_cms.z = -_vel_estimate_ned_ms.z * 100.0;
...
void AC_PosControl::update_U_controller()
{
    ...
    _vel_target_neu_cms.z = _p_pos_u.update_all(pos_target_zf, -_pos_estimate_ned_m.z * 100.0);
...
void AC_PosControl::get_stopping_point_NE_cm(Vector2p &stopping_point_neu_cm) const
{
    ...
    stopping_point_neu_cm = _pos_estimate_ned_m.xy() * 100.0;
    stopping_point_neu_cm -= _pos_offset_neu_cm.xy();

    Vector2f curr_vel = _vel_estimate_ned_ms.xy() * 100.0;
...
{
    float curr_pos_u_cm = -_pos_estimate_ned_m.z * 100.0;
    curr_pos_u_cm -= _pos_offset_neu_cm.z;

    float curr_vel_u_cms = -_vel_estimate_ned_ms.z * 100.0;
...
{
    return get_bearing_cd(Vector2f{0.0, 0.0}, (_pos_target_neu_cm.xy() - _pos_estimate_ned_m.xy() * 100.0).tofloat());
}
...
void AC_PosControl::standby_NEU_reset()
{
    ...
    _pos_target_neu_cm = _pos_estimate_ned_m * 100.0;
    _pos_target_neu_cm.z *= -1.0;
...
void AC_PosControl::write_log()
{
    if (is_active_NE()) {
        ...
        Write_PSCN(_pos_desired_neu_cm.x, _pos_target_neu_cm.x, _pos_estimate_ned_m.x * 100.0,
                   _vel_desired_neu_cms.x, _vel_target_neu_cms.x, _vel_estimate_ned_ms.x * 100.0,
                   _accel_desired_neu_cmss.x, _accel_target_neu_cmss.x, accel_n_cmss);
        Write_PSCE(_pos_desired_neu_cm.y, _pos_target_neu_cm.y, _pos_estimate_ned_m.y * 100.0,
                   _vel_desired_neu_cms.y, _vel_target_neu_cms.y, _vel_estimate_ned_ms.y * 100.0,
    ...
    if (is_active_U()) {
        Write_PSCD(-_pos_desired_neu_cm.z, -_pos_target_neu_cm.z, _pos_estimate_ned_m.z * 100.0,
                   -_vel_desired_neu_cms.z, -_vel_target_neu_cms.z, _vel_estimate_ned_ms.z * 100.0,
...
{
    const Vector2f pos_error = (_pos_target_neu_cm.xy() - _pos_estimate_ned_m.xy() * 100.0).tofloat();
...
void AC_PosControl::handle_ekf_NE_reset()
{
    ...
    if (reset_ms != _ekf_ne_reset_ms) {
        ...
        _pos_target_neu_cm.xy() = _pos_estimate_ned_m.xy() * 100.0 + _p_pos_ne.get_error().topostype();
        _pos_desired_neu_cm.xy() = _pos_target_neu_cm.xy() - _pos_offset_neu_cm.xy();
        _vel_target_neu_cms.xy() = _vel_estimate_ned_ms.xy() + _pid_vel_ne.get_error();
        _vel_desired_neu_cms.xy() = _vel_target_neu_cms.xy() - _vel_offset_neu_cms.xy();
...
void AC_PosControl::handle_ekf_U_reset()
{
    ...
    if (reset_ms != 0 && reset_ms != _ekf_u_reset_ms) {
        ...
        // To zero real position shift during relative position modes like Loiter, PosHold, Guided velocity and accleration control.
        _pos_target_neu_cm.z = -_pos_estimate_ned_m.z * 100.0 + _p_pos_u.get_error();
        _pos_desired_neu_cm.z = _pos_target_neu_cm.z - (_pos_offset_neu_cm.z + _pos_terrain_u_cm);
        _vel_target_neu_cms.z = -_vel_estimate_ned_ms.z * 100.0 + _pid_vel_u.get_error();
