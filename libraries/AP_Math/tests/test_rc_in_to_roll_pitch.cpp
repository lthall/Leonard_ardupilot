...
TEST(RC2RPTest, Corners) {
    ...
    for (uint i=0; i<ARRAY_SIZE(roll_val_deg); i++) {
        rc_input_to_roll_pitch(roll_out_deg, pitch_out_deg, roll_in[i], pitch_in[i], angle_max_deg, angle_limit_deg);
...
TEST(RC2RPTest, Circle) {
    ...
    for (uint row=0; row<ARRAY_SIZE(xy_rp); row++) {
        ...
        rc_input_to_roll_pitch(roll_out_deg, pitch_out_deg, roll_in, pitch_in, angle_max_deg, angle_limit_deg);
