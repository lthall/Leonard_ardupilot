...
void SCurve::calculate_track(const Vector3p &origin, const Vector3p &destination, float arc_rad,
                             ...
                             float snap_maximum, float jerk_maximum)
{
    ...
    if (!is_positive(chord_length) || fabsf(wrap_PI(arc_rad)) < radians(1.0)) {
        // straight segment
        is_arc_segment = false;
        arc_angle_rad  = 0.0f;
        ...
        const float turn_dir = (arc_angle_rad >= 0.0f) ? 1.0f : -1.0f; // -1 for CCW, 1 for CW
