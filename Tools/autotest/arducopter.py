...
""")
    ...
    def LUAConfigProfile(self):
        ...
        self.context_collect('STATUSTEXT')
        self.reboot_sitl()
        self.assert_prearm_failure('Reboot required for configuration change', other_prearm_failures_fatal=False)
        self.context_pop()
        self.reboot_sitl()
        self.wait_ready_to_arm()
        ...
        self.set_parameter("CFG_PAY_MODE", 22)
        self.wait_statustext("Invalid mode selected for PAY")
...
"""
        assert_prearm_failure_for_domains_string(domain_param_not_in_defaults, "Param Not in defaults")
...
"""
        assert_prearm_failure_for_domains_string(domain_param_not_in_defaults, "config_profiles: ACRO_BAL_PITCH exists in two")

        domain_param_must_be_set = """
local config_domains = {
   DOMAIN_A = {
      param_name = "DMN_A",
      param_mode_index = 4,
...
"""
        assert_prearm_failure_for_domains_string(domain_param_must_be_set, "config_profiles: ACRO_BAL_PITCH exists in DMN_A.al")  # noqa: E501
...
"""
        assert_prearm_failure_for_domains_string(
            domain_profile_param_same_as_domain_default_param_value,
            " has the same value as DMN_A.all"
        )
...
"""
        assert_prearm_failure_for_domains_string(
            domain_profile_param_must_be_set_when_mode_is_apply_defaults,
            "P2 in DMN_A is must-be-set but al"
        )
